...
const MAP_ONBOARDING_DISMISSED_KEY = "ne.map.onboarding.dismissed.v1";
const MAP_ONBOARDING_CHANGE_OPTION_LABEL = "Population (Change '21-23)";
...
export const createMapView = ({
  ...
  let scheduleOnboardingRefresh = () => {};
  let startOnboardingTour = () => {};

  // Minimal guided onboarding for the map stat chip flow.
  if (!isMobile) {
    ...
    try {
      onboardingDismissed = window.localStorage.getItem(MAP_ONBOARDING_DISMISSED_KEY) === "1";
    } catch {}
    ...
    const setOnboardingDismissed = () => {
      ...
      try {
        window.localStorage.setItem(MAP_ONBOARDING_DISMISSED_KEY, "1");
      } catch {}
    };
    ...
    const getSelectedStatChipTarget = (): HTMLElement | null =>
      categoryChips.element.querySelector('[data-ne-tour-target="map-primary-stat-chip"]');

    const getChangeOptionTarget = (): HTMLElement | null => {
      const optionEls = Array.from(
        categoryChips.element.querySelectorAll<HTMLElement>('[data-ne-tour-target="map-primary-stat-option"]'),
      );
      if (optionEls.length === 0) {
        return categoryChips.element.querySelector('[data-ne-tour-target="map-primary-stat-menu"]');
      }
      const normalizedPreferred = MAP_ONBOARDING_CHANGE_OPTION_LABEL.toLowerCase();
      const exact = optionEls.find(
        (option) =>
          (option.getAttribute("data-ne-tour-stat-label") ?? "").trim().toLowerCase() === normalizedPreferred,
      );
      if (exact) return exact;
      return (
        optionEls.find((option) => option.getAttribute("data-ne-tour-change-option") === "true") ??
        optionEls[0] ??
        null
      );
    };
    ...
    const showChipStep = (attempt = 0) => {
      ...
      if (!target) {
        // Wait much longer when tour is force-started (URL/settings) so initial
        // stat loading has time to render the selected-stat chip.
    ...
    startOnboardingTour = () => {
      ...
      // Restart from the initial welcome banner instead of jumping into step 1.
    ...
    window.addEventListener("resize", scheduleOnboardingRefresh);
    destroyFns.push(() => {
      clearOnboardingRetry();
      if (onboardingPositionRaf !== null) {
        cancelAnimationFrame(onboardingPositionRaf);
      ...
      window.removeEventListener("resize", scheduleOnboardingRefresh);
      hideTourStep();
      welcomeToast.remove();
      stepOverlay.remove();
    });
  ...
  if (poiDebugEnabled && typeof window !== "undefined") {
    (window as any).__poiDebugSnapshot = () => ({
      ...
      poiRowsZipAllScopes: getPointsOfInterestRows(pointsOfInterestSnapshot, "ZIP", null).length,
      poiRowsCounty: getPointsOfInterestRows(pointsOfInterestSnapshot, "COUNTY", null).length,
  ...
  const getZipPoiRowsForCurrentView = () => {
    const allRows = getPointsOfInterestRows(pointsOfInterestSnapshot, "ZIP", null);
  ...
  const getCountyPoiRowsForCurrentView = () => {
    const countyRowsAll = getPointsOfInterestRows(pointsOfInterestSnapshot, "COUNTY", null);
  ...
  const shouldRenderPointsOfInterest = () => extremasVisible;
  ...
  const buildHoverPillsByArea = (): {
    ...
    const appendPill = (
      ...
      // Avoid duplicate tooltip rows when a POI extremum and the selected-stat extremum
      // represent the same stat + direction in the same scope for one area.
      if (pill.statId) {
        const pillScope = pill.scopeLabel ?? null;
        const hasEquivalent = rows.some((row) =>
          row.statId === pill.statId
          && row.direction === pill.direction
          && (row.scopeLabel ?? null) === pillScope,
        );
  ...
  const updatePointsOfInterestSource = () => {
    ...
    poiDebugLog("poi-source-updated", {
      ...
      selectedCategory,
      zipPoiScopeMode: boundaryMode === "zips" ? "tulsa_area+okc_area" : "n/a",
      zipRowsAllScopes: getPointsOfInterestRows(pointsOfInterestSnapshot, "ZIP", null).length,
  ...
  const updateStatExtremaArrows = () => {
    ...
    const showZipStatLayers = showZipLayers;
    const showCountyStatLayers = showCountyLayers;
