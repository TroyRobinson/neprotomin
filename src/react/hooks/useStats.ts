...
export const useStats = ({
  ...
  const statDataQuery = useMemo(() => {
    if (!activeStatDataRequestKey) return null;
    return {
      statData: {
        $: {
          where: {
  ...
  useEffect(() => {
    if (!activeStatDataRequestKey) return;
    const batchKey = activeStatDataRequestKey;
  ...
  const observedBatchKeyRef = useRef<string>("");
  useEffect(() => {
    if (!activeStatDataRequestKey) {
      observedBatchKeyRef.current = "";
      return;
    }
    const batchKey = lastRequestedBatchKeyRef.current;
    if (!batchKey) return;

    // Instant's React hook can expose the previous query's snapshot for one commit
    // while useSyncExternalStore swaps subscriptions. Skip that commit so stale
    // rows are not processed as the newly requested batch.
    if (observedBatchKeyRef.current !== batchKey) {
      observedBatchKeyRef.current = batchKey;
      return;
    }
